...
async function spawnWorker(modelSnap, games, depth, forceRandomPlies, endgameExtraDepth, endgameTrigger, epsilon) {
...
async function main() {
    ...
    const endgameExtraDepth = Number(process.env.ENDGAME_EXTRA || 6);
    ...
    const endgameTrigger = Number(process.env.ENDGAME_TRIGGER || 10); // 残りマス数で発動

        const evaler = new NNEval({ hiddenSizes: [16], lr: 0.0001, lambda: 1e-6, optimizer: 'adam', beta1: 0.9, beta2: 0.999, eps: 1e-8, featureOpponentMoves: true });
    ...
    while (true) {
        ...
        for (let i = 0; i < threads; i++) {
            tasks.push(spawnWorker(snap, gamesPerWorker, depth, forceRandomPlies, endgameExtraDepth, endgameTrigger, epsilon));
        }
